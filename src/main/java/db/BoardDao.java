...
public class BoardDao {
	...
	public List<Board> listBoard(String field, String query, int page) {
		...
		String sql = "SELECT b.bid, b.uid, b.title, b.modTime, "
				+ "	b.viewCount, b.replyCount, u.uname FROM board AS b" + "	JOIN users AS u" + "	ON b.uid=u.uid"
				+ "	WHERE b.isDeleted=0 AND " + field + " LIKE ?" + "	ORDER BY bid DESC" + "	LIMIT 10 OFFSET ?;";
	...
	public Board getBoardDetail(int bid) {
		...
		String sql = "SELECT b.bid, b.uid, b.title, b.content, b.modTime, b.viewCount,"
				+ "	b.replyCount, b.files, u.uname FROM board AS b" + "	JOIN users AS u" + "	ON b.uid=u.uid"
				+ "	WHERE b.bid=?";
		Board b = new Board();
		try {
			PreparedStatement pStmt = conn.prepareStatement(sql);
